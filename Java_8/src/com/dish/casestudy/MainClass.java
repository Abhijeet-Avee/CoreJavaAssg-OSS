...
import com.dish.casestudy.Dish.Type;
...
public class MainClass{
	...
	public static void main(String[] args) {
		...
		//3. Get list of All Dishes only containing name and calValue			
			List<DishSelectedField> nameAndCalories = allDishes
														.stream()
														.map(d-> new DishSelectedField(d.getName(),d.getCalories()))
														.collect(toList());
		//	nameAndCalories.forEach(name->System.out.println(name));
			
		//4. Create a List by selecting the first three dishes that have more than 300 calories
			List<Dish> firstThree = allDishes
									.stream()
									.filter(dish->dish.getCalories()>300)
									.limit(3)
									.collect(Collectors.toList());
			//firstThree.forEach(dish->System.out.println(dish));
		...
		//8. Find if any food item is veg? findAny
			Optional<Dish> findAny = allDishes
									.stream()
									.filter(dish->dish.isVegetarian())
									.findAny();
		//	String result = findAny.map(Dish::getName).orElse("Not Found");
		
		//9. Primitive stream specializations

		// IntStream,DoubleStream, and LongStream==> avoid boxing cost

		//9.a. Get all the cal values of all food items
		...
		//10. Find max calories values for all dishes, Optional
